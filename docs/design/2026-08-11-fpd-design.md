...
connection preface → SETTINGS → WINDOW_UPDATE → HEADERS (HPACK-decoded via
`fluke-hpack`) → capture complete. Afterwards the connection is handed to a normal
...
| `terms` | — | manual | carve-out | terms text; acceptance record |
...
2. **Round-trip tests.** `emulate(profile)` → `parse` → equals `profile`. Closes the
